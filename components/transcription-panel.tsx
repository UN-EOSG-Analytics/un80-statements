            ...
            if (pollData.transcriptId) {
              try {
                const speakerResponse = await fetch(
                  "/api/get-speaker-mapping",
                  {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
                      transcriptId: pollData.transcriptId,
                    }),
                  },
                );
                if (speakerResponse.ok) {
    ...
    <div>
      ...
      {loading && (
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <div className="h-4 w-4 animate-spin rounded-full border-2 border-current border-t-transparent" />
          <span>
            Generating transcript... This may take several minutes for long
            videos.
          </span>
        </div>
      )}
      ...
      {!segments && !loading && !error && !checking && (
        <p className="text-sm text-muted-foreground">
          Click &quot;Generate Transcript&quot; to create a text transcript of
          this video using AI.
        </p>
      )}
    </div>
